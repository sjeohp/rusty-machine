...
impl UnSupModel<Matrix<f64>, Matrix<f64>> for PCA {
    fn predict(&self, inputs: &Matrix<f64>) -> LearningResult<Matrix<f64>> {
        match self.n_features {
            ...
            Some(f) => {
                if f != inputs.cols() {
                    return Err(Error::new(ErrorKind::InvalidData, "Input data must have the same number of columns as training data"));
                }
            }
        };
    ...
    fn train(&mut self, inputs: &Matrix<f64>) -> LearningResult<()> {
        match self.n {
            ...
            Some(n) => {
                if n > inputs.cols() {
                    return Err(Error::new(ErrorKind::InvalidData, "Input data must have equal or larger number of columns than n"));
                }
            }
        }
...
unsafe fn centering(inputs: &Matrix<f64>, centers: &Vector<f64>) -> Matrix<f64> {
    ...
    Matrix::from_fn(inputs.rows(), inputs.cols(), |r, c| inputs.get_unchecked([r, c]) - centers.data().get_unchecked(c))
}
